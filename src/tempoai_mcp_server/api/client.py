"""
API client for Tempo AI MCP Server.

This module handles all HTTP communication with the Tempo AI API,
...
async def _get_httpx_client() -> httpx.AsyncClient:
    ...
    try:
        server_module = sys.modules.get("tempoai_mcp_server.server")
...
def _get_error_message(error_code: int, error_text: str) -> str:
    ...
    error_messages = {
        ...
        HTTPStatus.INTERNAL_SERVER_ERROR: f"{HTTPStatus.INTERNAL_SERVER_ERROR.value} {HTTPStatus.INTERNAL_SERVER_ERROR.phrase}: The Tempo AI server encountered an internal error.",
        HTTPStatus.SERVICE_UNAVAILABLE: f"{HTTPStatus.SERVICE_UNAVAILABLE.value} {HTTPStatus.SERVICE_UNAVAILABLE.phrase}: The Tempo AI server might be down or undergoing maintenance.",
    }
    try:
...
def _prepare_request_config(
    ...
    """Prepare request configuration including headers and URL.

    Returns:
        Tuple of (full_url, headers, error_message).
    ...
    full_url = f"{config.tempo_ai_api_base_url}{url}"
    return full_url, headers, None
...
async def make_tempo_ai_request(
    ...
    """
    Make a GET request to the Tempo AI API with proper error handling.
    ...
    full_url, headers, error_msg = _prepare_request_config(url, api_key)
    if error_msg:
